...
def asc_to_safe(asc_file: str) -> tuple[dict, dict]:
    ...
    pns_params = hw_namespace_to_dict(hw)
    cardiac_params = None if hw_cardiac is None else hw_namespace_to_dict(hw_cardiac)

    return pns_params, cardiac_params


def get_random_safe_params():
    """Return example SAFE parameters for demo purposes."""
    return {
        'tau1': np.array([0.004, 0.004, 0.004]),
        'tau2': np.array([0.020, 0.020, 0.020]),
        'tau3': np.array([0.040, 0.040, 0.040]),
        'a1': np.array([0.4, 0.4, 0.4]),
        'a2': np.array([0.3, 0.3, 0.3]),
        'a3': np.array([0.3, 0.3, 0.3]),
        'stim_limit': np.array([2.0, 2.0, 2.0]),
        'g_scale': np.array([1.0, 1.0, 1.0]),
    }
